...
function simple_php_captcha($config = array()) {
    ...
    if (!empty($captcha_config['image_url'])) {
        $image_src = rtrim($captcha_config['image_url'], '?&') . '?_CAPTCHA=1&t=' . urlencode(microtime());
    } elseif ( strpos($_SERVER['SCRIPT_FILENAME'], $_SERVER['DOCUMENT_ROOT']) ) {
        $image_src = substr(__FILE__, strlen( realpath($_SERVER['DOCUMENT_ROOT']) )) . '?_CAPTCHA=1&t=' . urlencode(microtime());
        $image_src = '/' . ltrim(preg_replace('/\\\\/', '/', $image_src), '/');
    } else {
        $_SERVER['WEB_ROOT'] = str_replace($_SERVER['SCRIPT_NAME'], '', $_SERVER['SCRIPT_FILENAME']);
        $image_src = substr(__FILE__, strlen( realpath($_SERVER['WEB_ROOT']) )) . '?_CAPTCHA=1&t=' . urlencode(microtime());
        $image_src = '/' . ltrim(preg_replace('/\\\\/', '/', $image_src), '/');
    }
...
if (!function_exists('simple_php_captcha_image_data')) {
    function simple_php_captcha_image_data($captcha_config) {
...
if (!function_exists('simple_php_captcha_inline')) {
    function simple_php_captcha_inline($config = array()) {
        $captcha = simple_php_captcha($config);
        $captcha_config = unserialize($_SESSION['_CAPTCHA']['config'] ?? '');

        if (!$captcha_config) {
            throw new Exception('CAPTCHA configuration is missing');
        }

        $captcha['image_src'] = 'data:image/png;base64,' . base64_encode(simple_php_captcha_image_data($captcha_config));
...
if( isset($_GET['_CAPTCHA']) ) {
    ...
    echo simple_php_captcha_image_data($captcha_config);
    exit();
