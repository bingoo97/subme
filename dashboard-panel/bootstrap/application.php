...
if (!function_exists('app_static_page_normalize_locale')) {
    ...
    {
        $localeCode = localization_normalize_locale((string)$localeCode);
        return in_array($localeCode, ['pl', 'en'], true) ? $localeCode : 'en';
    }
}

if (!function_exists('app_static_page_locale_slug')) {
    function app_static_page_locale_slug(string $baseSlug, ?string $localeCode): string
    {
        return trim($baseSlug) . '-' . app_static_page_normalize_locale($localeCode);
    }
}

if (!function_exists('app_ensure_static_pages_runtime_columns')) {
    function app_ensure_static_pages_runtime_columns(Mysql_ks $db): void
    {
        static $done = false;
...
if (!function_exists('app_instruction_guides_seed_data')) {
    function app_instruction_guides_seed_data(array $messages, bool $applicationInstructionsEnabled = true): array
    {
        $guides = [
            [
...
if (!function_exists('app_apps_seed_data')) {
    function app_apps_seed_data(array $messages, bool $applicationInstructionsEnabled = true): array
    {
        $apps = [
            [
                'name' => 'NewLook 4',
                'logo' => '/img/new_look.png',
                'url' => 'https://tinyurl.com/3av56p84',
                'instruction_url' => '/instruction-newlook',
            ],
            [
                'name' => 'NewLook 2',
                'logo' => '/img/new_look.png',
                'url' => 'https://tinyurl.com/yc37sjfa',
                'instruction_url' => '/instruction-newlook',
            ],
            [
                'name' => 'OTT-Player',
                'logo' => '/img/ott/logo-ott.jpg',
                'url' => 'https://tinyurl.com/46c2hb3s',
...
if (!function_exists('app_render_instruction_guides_markup')) {
    function app_render_instruction_guides_markup(array $instructionGuides): string
    {
        ob_start();
        ?>
<div class="home_buttons home_buttons--compact">
    <?php foreach ($instructionGuides as $guide): ?>
        <?php
...
if (!function_exists('app_render_apps_table_markup')) {
    function app_render_apps_table_markup(array $apps, array $messages): string
    {
        ob_start();
        ?>
<table class="table table-bordered table-striped">
    <thead>
        <tr class="center">
            ...
            <td class="text-left"><?php echo htmlspecialchars(localization_translate($messages, 'col_name', 'Name'), ENT_QUOTES, 'UTF-8'); ?></td>
            <td></td>
        </tr>
    </thead>
    <tbody>
    <?php foreach ($apps as $app): ?>
        ...
        $logo = htmlspecialchars((string)($app['logo'] ?? ''), ENT_QUOTES, 'UTF-8');
        $url = trim((string)($app['url'] ?? ''));
        $instructionUrl = trim((string)($app['instruction_url'] ?? ''));
        ?>
        <tr align="center">
            <td style="width: 50px;"><img src="<?php echo $logo; ?>" alt="<?php echo $name; ?>" class="rounded" width="32" height="32"></td>
            <td class="text-left">
                <?php if ($instructionUrl !== ''): ?>
                    <a href="<?php echo htmlspecialchars($instructionUrl, ENT_QUOTES, 'UTF-8'); ?>"><?php echo $name; ?></a>
                <?php else: ?>
                    <?php echo $name; ?>
                <?php endif; ?>
            </td>
            <td>
                <?php if ($url !== ''): ?>
                    <a href="<?php echo htmlspecialchars($url, ENT_QUOTES, 'UTF-8'); ?>" target="_blank" rel="noopener noreferrer" class="text-dark">
...
if (!function_exists('app_render_apps_fallback_link_markup')) {
    function app_render_apps_fallback_link_markup(string $appsUrl, bool $fallbackEnabled, array $messages): string
    {
        $appsUrl = trim($appsUrl);
        if ($appsUrl === '' || !$fallbackEnabled) {
            return '';
        }

        return '<p>'
            . htmlspecialchars(localization_translate($messages, 'apps_download_from', 'You can download apps directly from'), ENT_QUOTES, 'UTF-8')
            . ': <a href="'
            . htmlspecialchars($appsUrl, ENT_QUOTES, 'UTF-8')
...
if (!function_exists('app_static_page_body_instructions')) {
    function app_static_page_body_instructions(array $messages): string
    {
        $title = htmlspecialchars(localization_translate($messages, 'instructions_title', 'Instructions'), ENT_QUOTES, 'UTF-8');
        $intro = htmlspecialchars(localization_translate($messages, 'instructions_intro', 'Choose the payment guide you want to open.'), ENT_QUOTES, 'UTF-8');

        return <<<HTML
<div class="content-box info-page">
    <div class="payment-wizard__header">
        <h1><a href="/"><i class="fa fa-chevron-circle-left back" aria-hidden="true"></i></a> {$title}</h1>
        <p>{$intro}</p>
    </div>
...
if (!function_exists('app_static_page_body_apps')) {
    function app_static_page_body_apps(array $messages): string
    {
        $title = htmlspecialchars(localization_translate($messages, 'apps_title', 'Apps'), ENT_QUOTES, 'UTF-8');
        $intro = htmlspecialchars(localization_translate($messages, 'apps_intro', 'Choose an app for your device and open its official download page. After installation, use your login details or stream link from the subscription modal.'), ENT_QUOTES, 'UTF-8');
...
if (!function_exists('app_static_page_body_apps_materialized')) {
    function app_static_page_body_apps_materialized(array $messages, bool $applicationInstructionsEnabled = true): string
    {
        return app_render_static_page_body(
            app_static_page_body_apps($messages),
            [
                'apps_table' => app_render_apps_table_markup(
                    app_apps_seed_data($messages, $applicationInstructionsEnabled),
                    $messages
                ),
            ]
        );
...
if (!function_exists('app_static_page_body_instruction_crypto_exchange')) {
    function app_static_page_body_instruction_crypto_exchange(): string
    {
        return <<<'HTML'
...
<div class="alert alert-danger"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> <strong>UWAGA!</strong> W przypadku płatności z giełdy kryptowalutowej należy podać imię i nazwisko osoby do której jest wykonywany przelew w zależności od wymogów giełdy. Prosimy nie wpisywać nic na temat Subskrybcji czy opłaty za Stream lub TV np. {{site_name}} lub IPTV itp.</div>
HTML;
    }
}

if (!function_exists('app_static_page_body_instruction_revolut')) {
    function app_static_page_body_instruction_revolut(array $messages): string
    {
        $downloadButton = htmlspecialchars(localization_translate($messages, 'instruction_revolut_download_button', 'Pobierz aplikację Revolut'), ENT_QUOTES, 'UTF-8');
        $howToPayTitle = htmlspecialchars(localization_translate($messages, 'instruction_revolut_how_to_pay_title', 'Jak zapłacić?'), ENT_QUOTES, 'UTF-8');
        $note = htmlspecialchars(localization_translate($messages, 'instruction_revolut_note', '*** Płatność z Revolut jest realizowana zazwyczaj w ciągu 5-30 min od chwili wysłania. Poinformuj nas na Live Chat o dokonanej transakcji i podaj szczegóły transferu.'), ENT_QUOTES, 'UTF-8');
        $alert1 = htmlspecialchars(localization_translate($messages, 'instruction_revolut_alert_1', 'Jeśli nie jesteś pewny, ile dokładnie kupić, napisz do nas na LIVE CHAT, a wyliczymy to za Ciebie.'), ENT_QUOTES, 'UTF-8');
        $alert2 = htmlspecialchars(localization_translate($messages, 'instruction_revolut_alert_2', 'Sprawdź przed wysłaniem, ile wynosi opłata transakcyjna! Ostateczne prowizje i cały koszt transakcji zobaczysz po wpisaniu kwoty oraz naszego adresu do wysyłki w podsumowaniu transakcji przed wysłaniem. Jeśli masz wątpliwości, to napisz do nas!'), ENT_QUOTES, 'UTF-8');
        $intro1 = localization_translate($messages, 'instruction_revolut_intro_1', '<strong>Revolut</strong> jest międzynarodowym bankiem i jedną z najpopularniejszych aplikacji płatniczych na świecie.');
        $intro2 = htmlspecialchars(localization_translate($messages, 'instruction_revolut_intro_2', 'Dostęp do kryptowalut w aplikacji jest już od kilku lat, a w ostatnim czasie aplikacja udostępniła możliwość wysyłania kryptowalut na zewnętrzne adresy i szybkie płatności kryptowalutowe między użytkownikami.'), ENT_QUOTES, 'UTF-8');

        return <<<HTML
<img src="img/rev-crypto.jpg" class="img-responsive" alt="img" />
<br />
<p class="text-justify">{$intro1}</p>
<p class="text-justify">{$intro2}</p>
<hr/>
<a href="https://revolut.com/referral/?referral-code=marcinuw1!FEB2-24-AR-L1" class="btn btn-warning btn-lg w-100 mb-3" target="_blank" title="download">
    {$downloadButton} <i class="fa fa-angle-double-right" aria-hidden="true"></i>
</a>
<br />
<h1 class="home_title">{$howToPayTitle}</h1>
<ul style="list-style: none;padding: 5px;">
    <li>1. Załóż darmowe konto w Revolut<br/> - <a class="strong blue" title="" href="https://revolut.com/referral/?referral-code=marcinuw1!FEB2-24-AR-L1" target="_blank">Link do Revolut</a></li>
    <li>2. Doładuj konto bankowe w Revolut za pomocą <strong>ApplePay, PayPal lub kartą VISA, Mastercard.</strong></li>
    <img src="/img/crypto/revo/1.jpeg" class="img-responsive phone-img" alt="" />
    <li>3. Przejdź: <span class="btn btn-xs btn-outline-dark strong">Krypto</span> <i class="fa fa-caret-right" aria-hidden="true"></i>  <span class="btn btn-xs btn-outline-dark strong">Inwestuj</span></li>
    <img src="/img/crypto/revo/2.jpeg" class="img-responsive phone-img" alt="" />
    <li>4. Wybierz kryptowalutę, którą chcesz zakupić np. <span class="btn btn-xs btn-outline-dark strong">Bitcoin</span> <span class="btn btn-xs btn-outline-dark strong">Dogecoin</span> <br />- <strong class="red">napisz do nas przed płatnością, aby dokonać wyliczenia kwoty do zakupu.</strong> <span class="btn btn-xs btn-danger strong"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i></span></li>
    <img src="/img/crypto/revo/3.jpeg" class="img-responsive phone-img" alt="" />
    <li>5. Wpisz kwotę zakupu <br/><strong class="red border-red border-radius text-justify">UWAGA! Revolut pobiera prowizję w przypadku kryptowalut - należy kupić za przynajmniej 2-3 EUR więcej niż cena zamówienia. Jeśli chcesz wysyłać kryptowaluty bez prowizji operatora, przejdź na TrustWallet. Tam prowizja za przesłanie to zaledwie 0.10$.</strong></li>
    <img src="/img/crypto/revo/4.jpeg" class="img-responsive phone-img" alt="" />
    <li>5. Dokonaj zakupu.</li>
    <li>6. Poczekaj na zatwierdzenie transakcji.</li>
    <li>7. Wybierz: <span class="btn btn-xs btn-outline-dark strong">Wyślij / Send</span></li>
    <img src="/img/crypto/revo/5.jpeg" class="img-responsive phone-img" alt="" />
    <li>8. Wybierz: <span class="btn btn-xs btn-outline-dark strong">Dodaj adres portfela</span><br/><strong class="red border-red border-radius text-justify mt-2"> W przypadku płatności przez Revolut lub z giełdy kryptowalutowej należy podać imię i nazwisko osoby, do której jest wykonywany przelew.</strong></li>
    <img src="/img/crypto/revo/6.jpeg" class="img-responsive phone-img" alt="" />
    <li>9. Wybierz: <span class="btn btn-xs btn-outline-danger strong">Portfel innej osoby <i class="fa fa-caret-right" aria-hidden="true"></i> TrustWallet</span></li>
    <li>- Imię i nazwisko: <span class="btn btn-xs btn-outline-danger strong">otrzymasz od nas na Live Chat!</span></li>
    <img src="/img/crypto/revo/7.jpeg" class="img-responsive phone-img" alt="" />
    <li>10. Wpisz <strong>adres portfela</strong>, który otrzymasz od nas. <span class="btn btn-xs btn-danger strong"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i></span></li>
    <li>11. Wpisz <strong>nazwę portfela</strong> np. portfel zewnętrzny (opcjonalnie)<br /><strong class="red border-red border-radius">UWAGA! Prosimy nie wpisywać nic na temat subskrypcji czy opłaty za Stream lub TV np. {{site_name}} lub IPTV itp.</strong></li>
    <li>12. Wybierz dodany wcześniej portfel z listy dostępnych portfeli.</li>
    <li>13. Wpisz <strong>kwotę transakcji</strong><br />- <strong class="red">napisz do nas przed płatnością, aby dokonać wyliczeń liczby tokenów do wysłania.</strong> <span class="btn btn-xs btn-danger strong"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i></span></li>
    <li>14. Zatwierdź klikając: <span class="btn btn-xs btn-outline-dark strong">Wyślij</span></li>
    <li>15. Gratulacje! Dokonałeś płatności!</li>
</ul>
<p class="text-muted">{$note}</p>
<hr/>
<div class="alert alert-info"><i class="fa fa-info-circle" aria-hidden="true"></i> {$alert1}</div>
<div class="alert alert-info"><i class="fa fa-info-circle" aria-hidden="true"></i> {$alert2}</div>
HTML;
    }
}

if (!function_exists('app_static_page_body_instruction_ott_player')) {
    function app_static_page_body_instruction_ott_player(): string
    {
        return <<<'HTML'
...
if (!function_exists('app_static_page_body_instruction_smart_iptv')) {
    function app_static_page_body_instruction_smart_iptv(): string
    {
        return <<<'HTML'
...
if (!function_exists('app_static_page_body_instruction_newlook')) {
    function app_static_page_body_instruction_newlook(): string
    {
        return <<<'HTML'
...
if (!function_exists('app_static_page_body_instruction_trust_wallet')) {
    function app_static_page_body_instruction_trust_wallet(array $messages): string
    {
        $intro1 = htmlspecialchars(localization_translate($messages, 'instruction_trust_wallet_intro_1', 'Dokonaj płatności kryptowalutami za Pakiet. Zachęcamy do pobrania darmowej aplikacji TrustWallet na telefon. Bank Revolut połączył się z TrustWallet - teraz możesz kupić kryptowaluty, płacąc z salda Revolut lub kartą przez ApplePay lub GooglePay.'), ENT_QUOTES, 'UTF-8');
        $intro2 = htmlspecialchars(localization_translate($messages, 'instruction_trust_wallet_intro_2', 'Po instalacji postępuj zgodnie z instrukcjami, aby utworzyć nowy portfel, a następnie doładuj saldo konta np. kartą płatniczą.'), ENT_QUOTES, 'UTF-8');
        $infoAlert = htmlspecialchars(localization_translate($messages, 'instruction_trust_wallet_info_alert', 'Aplikacja TrustWallet obsługuje płatności przez ApplePay lub GooglePay oraz płatności z Revolut bez dodatkowych weryfikacji.'), ENT_QUOTES, 'UTF-8');
        $downloadButton = htmlspecialchars(localization_translate($messages, 'instruction_trust_wallet_download_button', 'Download TrustWallet'), ENT_QUOTES, 'UTF-8');
        $download = htmlspecialchars(localization_translate($messages, 'instruction_trust_wallet_download', 'Pobierz'), ENT_QUOTES, 'UTF-8');
        $title = htmlspecialchars(localization_translate($messages, 'instruction_trust_wallet_payment_guide_title', 'Instrukcja płatności'), ENT_QUOTES, 'UTF-8');

        return <<<HTML
<img src="/img/tw2.png" class="img-responsive" alt="Trust Wallet" />
<br />
<p class="strong text-justify">{$intro1}</p>
<p class="strong">{$intro2}</p>
<hr />
...
<div class="alert alert-dismissible alert-info">
    <i class="fa fa-info-circle" aria-hidden="true"></i> {$infoAlert}
</div>
<div class="instruction-content">
    <h1>{$title}</h1>
    <p class="desc-title">1. Pobierz aplikację TrustWallet - <a href="https://trustwallet.com/" target="_blank" class="strong" title="">{$download}</a></p>
    <p class="desc-note">Aby kupić kryptowalutę potrzebujesz portfela. TrustWallet jest darmową aplikacją na telefon z Android lub iOS. Po instalacji postępuj zgodnie z instrukcjami aby utworzyć nowy portfel.</p>
    <p class="desc-title">2. Następnie kliknij w przycisk "Środki" lub "Zasil swój portfel"</p>
    <img src="/img/crypto/trust/1.jpg" class="img-responsive phone-img" alt="" />
    <p class="desc-title">3. Wybierz formę płatności</p>
    <p class="desc-note">Wybierz płatność przez ApplePay, GooglePay lub Revolut. Środki zostaną pobrane natychmiast po zatwierdzeniu.</p>
    <img src="/img/crypto/trust/2.jpg" class="img-responsive phone-img" alt="" />
    <p class="desc-title">4. Wybierz kwotę i kryptowalutę</p>
    <p class="desc-note">Wybierz kryptowalutę <strong>np. Bitcoin</strong> lub <strong>Dogecoin - DOGE</strong> ma bardzo niską opłatę za wysłanie (ok. 0.10$). Wpisz kwotę za jaką chcesz kupić pakiet. Zalecamy płatności za 3-6 miesięcy aby uniknąć częstych płatności.</p>
    <img src="/img/crypto/trust/3.jpg" class="img-responsive phone-img" alt="" />
    <p class="desc-title">5. Wybierz operatora płatności</p>
    <p class="desc-note">Wybierz operatora płatności np. Revolut. Zostaniesz przekierowany do strony gdzie zalogujesz się i potwierdzisz płatność. Po dokonaniu płatności otrzymasz potwierdzenie na email. Środki pojawią się w portfelu w ciągu 5-15 minut.</p>
    <img src="/img/crypto/trust/4.jpg" class="img-responsive phone-img" alt="" />
    <p class="desc-title">6. Zapłać za Pakiet</p>
    <p class="desc-note">Środki zostaną dodane do salda TrustWallet w ciągu 5-15 min.</p>
    <p class="desc-title">7. Wyślij przelew na nasz adres portfela</p>
    <p class="desc-note">Kliknij na stronie głównej w zakupioną kryptowalutę np. Dogecoin a następnie gdy zobaczysz swoje saldo zakupionej kryptowaluty kliknij w przycisk "Send".</p>
    <p class="desc-title">8. Wprowadź dane odbiorcy</p>
    <p class="desc-note">Po kliknięciu w przycisk "Send" wprowadź nasze dane, na które chcesz przesłać płatność.<br/>Adres oraz kwotę płatności otrzymasz od nas - napisz do nas na LIVE CHAT. Skopiuj dane do płatności albo zeskanuj QR kod w swoim portfelu kryptowalutowym.</p>
    <img src="/img/crypto/trust/6.jpg" class="img-responsive phone-img" alt="" />
    <p class="desc-title">9. Gratulacje! Właśnie zapłaciłeś za swój Pakiet!</p>
</div>
<hr />
<div class="d-grid gap-2">
    <a href="https://trustwallet.com/" target="_blank" class="btn btn-dark btn-lg border-radius">
        <i class="fa fa-external-link" aria-hidden="true"></i> {$downloadButton}
    </a>
    {{support_button_trust}}
</div>
HTML;
    }
}
...
if (!function_exists('app_ensure_system_content_pages_runtime')) {
    function app_ensure_system_content_pages_runtime(Mysql_ks $db): void
    {
        static $done = false;
        if ($done || !schema_object_exists($db, 'static_pages')) {
            return;
        ...
        foreach (['pl', 'en'] as $localeCode) {
            $messages = [];
            if (!function_exists('localization_load')) {
                $localizationBootstrap = dirname(__DIR__) . '/bootstrap/localization.php';
                if (is_file($localizationBootstrap)) {
                    require_once $localizationBootstrap;
                }
            }

            if (function_exists('localization_load')) {
                $messages = localization_load($localeCode, dirname(__DIR__))['messages'] ?? [];
            } else {
                $localeFile = dirname(__DIR__) . '/locales/' . $localeCode . '.php';
                $baseLocaleFile = dirname(__DIR__) . '/locales/en.php';
                $baseMessages = is_file($baseLocaleFile) ? (require $baseLocaleFile) : [];
                $localeMessages = is_file($localeFile) ? (require $localeFile) : [];
                $messages = array_replace(
                    is_array($baseMessages) ? $baseMessages : [],
                    is_array($localeMessages) ? $localeMessages : []
                );
            }
            ...
            foreach ($rows as $row) {
                $safeSlug = $db->escape((string)$row['slug']);
...
if (!function_exists('app_static_page_find_for_route')) {
    function app_static_page_find_for_route(Mysql_ks $db, string $baseSlug, ?string $localeCode): ?array
    {
        if (!schema_object_exists($db, 'static_pages')) {
            return null;
        }
...
if (!function_exists('app_render_static_page_body')) {
    function app_render_static_page_body(string $body, array $replacements = []): string
    {
        $renderedBody = $body;
        foreach ($replacements as $token => $replacement) {
            $renderedBody = str_replace('{{' . $token . '}}', (string)$replacement, $renderedBody);
        }

        return preg_replace('/\{\{[a-z0-9_\-]+\}\}/i', '', $renderedBody) ?? $renderedBody;
    }
}
...
{
    if ($walletId <= 0 || $customerId <= 0 || !schema_object_exists($db, 'crypto_wallet_assignments')) {
        return false;
    }
...
): int
{
    ...
    $updatedWallet = $db->update_using_id(
    ...
    $row = $db->select_user(
        "SELECT
         ...
         WHERE crypto_wallet_assignments.customer_id = {$customerId}
           ...
           AND NOT EXISTS (
                SELECT 1
                FROM crypto_wallet_assignments AS conflicting_assignment
                WHERE conflicting_assignment.wallet_address_id = crypto_wallet_assignments.wallet_address_id
                  AND conflicting_assignment.customer_id <> crypto_wallet_assignments.customer_id
                  AND conflicting_assignment.status IN ('reserved', 'active')
                LIMIT 1
           )
           AND NOT EXISTS (
                SELECT 1
                FROM crypto_deposit_requests AS foreign_request
                LEFT JOIN crypto_wallet_assignments AS foreign_payment_assignment
                  ON foreign_payment_assignment.id = foreign_request.wallet_assignment_id
                WHERE foreign_request.customer_id <> crypto_wallet_assignments.customer_id
                  AND (
                        foreign_request.wallet_address_id = crypto_wallet_assignments.wallet_address_id
                     OR foreign_payment_assignment.wallet_address_id = crypto_wallet_assignments.wallet_address_id
                  )
                  AND foreign_request.status NOT IN ('pending', 'pending_payment', 'cancelled')
                LIMIT 1
           )
         ORDER BY
...
function app_database_backup_directory(): string
{
    return app_backend_root_path() . '/storage/database-backups';
}

function app_database_backup_latest_path(): string
{
    return app_database_backup_directory() . '/latest-database-backup.sql';
}

function app_database_backup_meta_path(): string
{
    return app_database_backup_directory() . '/latest-database-backup.json';
}

function app_database_backup_ensure_directory(): bool
{
    $directory = app_database_backup_directory();
...
function app_format_file_size(int $bytes): string
{
    if ($bytes <= 0) {
        return '0 B';
    }
...
function app_database_backup_latest_info(): array
{
    $path = app_database_backup_latest_path();
    $meta = [];
    $metaPath = app_database_backup_meta_path();
    if (is_file($metaPath)) {
    ...
    if ($generatedAt === '' && $exists) {
        ...
        if (is_int($mtime) && $mtime > 0) {
            $generatedAt = date('Y-m-d H:i:s', $mtime);
        }
    }

    $downloadName = trim((string)($meta['download_name'] ?? ''));
    if ($downloadName === '' && $generatedAt !== '') {
        $downloadName = 'database-backup-' . date('Ymd-His', strtotime($generatedAt)) . '.sql';
    } elseif ($downloadName === '') {
        $downloadName = 'latest-database-backup.sql';
    }
    ...
    return [
        ...
        'size_label' => app_format_file_size($sizeBytes),
    ];
}

function app_stream_database_sql_backup_file(string $absolutePath, string $downloadFilename): void
{
    ...
    header('Content-Type: application/sql; charset=utf-8');
    header('Content-Length: ' . (string)((int)@filesize($absolutePath)));
    header('Content-Disposition: attachment; filename="' . str_replace('"', '', $downloadFilename) . '"');
    header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
    header('Pragma: no-cache');
...
function app_write_database_sql_backup(Mysql_ks $db, callable $writer, ?callable $progressCallback = null): void
{
    $objects = app_sql_dump_objects($db);
    $timestamp = date('Y-m-d H:i:s');
    $totalObjects = max(1, count($objects['tables']) + count($objects['views']));
    $processedObjects = 0;

    $emitProgress = static function (string $message) use ($progressCallback, &$processedObjects, $totalObjects): void {
        if ($progressCallback === null) {
            return;
        }

        $percent = min(96, 5 + (int)floor(($processedObjects / $totalObjects) * 90));
        $progressCallback([
            'percent' => $percent,
            'message' => $message,
    ...
    $writer("-- Generated by Subme admin backup\n");
    $writer('-- Export time: ' . $timestamp . "\n");
    ...
    foreach ($objects['tables'] as $tableName) {
        ...
        if ($createSql === '') {
            $processedObjects++;
            $emitProgress('Skipping table ' . $tableName . '...');
            continue;
        }

        $writer('DROP TABLE IF EXISTS ' . app_sql_dump_identifier($tableName) . ";\n");
        $writer($createSql . ";\n\n");

        $columns = app_sql_dump_insertable_columns($db, $tableName);
        if ($columns === []) {
            $processedObjects++;
            $emitProgress('Prepared table ' . $tableName . '.');
            continue;
        }
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $writer(
                'INSERT INTO ' . app_sql_dump_identifier($tableName)
    ...
    foreach ($objects['views'] as $viewName) {
        ...
        if ($createSql === '') {
            $processedObjects++;
            $emitProgress('Skipping view ' . $viewName . '...');
            continue;
        }
        ...
        $writer('DROP VIEW IF EXISTS ' . app_sql_dump_identifier($viewName) . ";\n");
        $writer(trim((string)$createSql) . ";\n\n");
        $processedObjects++;
        $emitProgress('Prepared view ' . $viewName . '.');
    }

    $writer("SET FOREIGN_KEY_CHECKS = 1;\n");
}

function app_generate_database_sql_backup_file(Mysql_ks $db): array
{
    if (!app_database_backup_ensure_directory()) {
        return [
            'ok' => false,
            'message' => 'Unable to prepare the backup directory.',
        ];
    }
    ...
    $directory = app_database_backup_directory();
    $latestPath = app_database_backup_latest_path();
    $metaPath = app_database_backup_meta_path();
    $tempPath = $directory . '/latest-database-backup.tmp';
    ...
    if ($handle === false) {
        return [
            'ok' => false,
            'message' => 'Unable to create the temporary SQL backup file.',
        ];
    }
    ...
    try {
        ...
        return [
            'ok' => false,
            'message' => 'Unable to generate the SQL backup: ' . trim($throwable->getMessage()),
        ];
    }
    ...
    if (!@rename($tempPath, $latestPath)) {
        @unlink($tempPath);
        return [
            'ok' => false,
            'message' => 'Unable to move the generated SQL backup into place.',
        ];
    }

    foreach ((array)glob($directory . '/*.sql') as $candidatePath) {
        if ($candidatePath !== $latestPath) {
            @unlink($candidatePath);
        }
    }

    $sizeBytes = (int)@filesize($latestPath);
    ...
    app_update_manual_database_backup_timestamp($db, $generatedAt);
